    ...
    public void AddMenuToDay(string dayAbbreviation, List<MenuItem> list)
{
    ...
    dayAbbreviation = char.ToUpper(dayAbbreviation[0]) + dayAbbreviation.Substring(1,1).ToLower();

    if (Items.ContainsKey(dayAbbreviation))
    {
        Items[dayAbbreviation] = list;
    }
    else
    {
        Items.Add(dayAbbreviation, list);
    }
}
